...
def render_graphs(store, now_jst, outdir, section="modes", prefix="graph_f", unit="Hz", with_theory=True):
    ...
    S = 2
    GW, GH = 1000 * S, 206 * S
    # 左は目盛り3つ分 (≈3時間) 空ける: プロットが画像左端に張り付かないように。
    # 右は目盛りラベルがちょうど収まる幅まで詰める
    PADL, PADR, PADT, PADB = 44 * S, 44 * S, 12 * S, 32 * S
    ...
    for key, name, theory, col in MODES:
        ...
        for hh in range(0, 73):
            ...
            if t.hour % 3 == 0 and hh < 72:
                anc = "la" if xx < 8 * S else "ma"  # 画像端に切れる場合のみ左寄せ
        ...
        # PNGは通常作らない (2026-08-05〜 数値JSONだけ運ぶ。ページ側がcanvasで描画)
        if os.environ.get("RENDER_PNG"):
